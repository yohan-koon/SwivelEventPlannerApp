...
export const AuthenticationStoreModel = types
  ...
  .props({
    ...
    isInitialImageUplaoded: types.optional(types.boolean, false),
    isInitialKYCUpdated: types.optional(types.boolean, false),
  })
  .actions(withSetPropAction)
  .actions((self) => ({
    async setAuthState(isLoggedIn: boolean, isInitialImageUplaoded: boolean, isInitialKYCUpdated: boolean, user: any){
      self.user = user || UserModel.create({})
      self.isLoggedIn = isLoggedIn
      self.isInitialImageUplaoded = isInitialImageUplaoded
      self.isInitialKYCUpdated = isInitialKYCUpdated
    },
  ...
  .actions((self) => ({
    ...
    //   try{
    //     if(!firebaseAuthUser){
    //       self.setAuthState(false,false, false, null)
    //     }else if(firebaseAuthUser){
    //       // Check if user exists in Firestore
    //       const firestoreUser: User = await fetchUser(firebaseAuthUser.uid);
    //       if(!firestoreUser){
    //         self.setAuthState(true, false, false, null)
    //       }else{
    //         self.setAuthState(true, firestoreUser?.profileImage && firestoreUser?.profileImage !== '' , firestoreUser?.firstName && firestoreUser?.firstName != '' ? true : false , firestoreUser)
    //       }
    //     }
    //   }catch(error){
